...
class AdminController extends Controller
{
    public function index()
    {
        if (Auth::check()) {
            ...
            if ($user) {
                ...
                return view('tickets.dispticketAdmin', [
                    'tickets' => $tickets,
    ...
    //     $user->save();
    //     return redirect()->route('admin.page', ['token' => $token]);
    // }
    ...
    public function updateTicket(TicketUpdateRequest $request, $id)
    {
        if (Auth::check()) {
            ...
            // Find and update the ticket in sender's tickets
            ...
            return redirect('/admin')->with('token', $senderToken);
        } else {
            return view('home.land');
        }
    }
    ...
    public function deleteTicket($id)
    {
        if (Auth::check()) {
            ...
            $sender->save();
            $tickets = json_decode($receiverUser->mytickets, true);
            $time = $receiverUser['created_at']->toDateTimeString();
            $notifications = json_decode($receiverUser->notifications, true);
            if ($notifications !== null) {
    ...
    public function markAllRead($token)
    {
        $user = User::where('token', $token)->first();
        $realNotifications = json_decode($user->notifications, true);
